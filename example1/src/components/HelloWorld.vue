    ...
    <el-col :span="6" class="right-col">
      <div class="panel">
        ...
        <el-select size="small" style="width:100%" v-model="remarkIndex" filterable placeholder="请选择" @change="remarkChange">
          <el-option v-for="(item, index) in remarkList" :key="index" :label="item | getRemarkText(index)" :value="index"> </el-option>
        </el-select>
        <el-form size="small" class="remark-form" v-if="remarkIndex !== ''">
          ...
          <el-form-item label="标注内容" v-if="remark.type === 'other'">
...
const remarkTypes = [
  ...
  { type: 'other', label: '自定义' }
]
export default {
  filters: {
    getRemarkText(item, index) {
      let res = item.data.content
      if (item.type !== 'other') {
        res = remarkTypes.filter(it => it.type === item.type)[0].label
      }
      return index + 1 + '.' + (res || '')
    }
  },
